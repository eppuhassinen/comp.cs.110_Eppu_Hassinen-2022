...
using namespace std;
using Funk = double(*)(double, double);

const string GREETING_AT_END = "Thanks and see you later!";
const string WRONG_NUMBER_OF_PARAM = "Error: wrong number of parameters.";
const string ERROR_NOT_A_NUMBER = "Error: a non-number operand.";
const string UNKWOWN_COMMAND = "Error: unknown command.";
...
double calculate(Funk f, double left = 0, double right = 0);
...
int main() {
    ...
    while ( true ) {
        ...
        for (char& c : pieces.at(0))
        {
            command_to_be_executed.push_back(toupper(c));
        }

        Command current_command = {"", 0, false, nullptr};
        ...
        if (current_command.str == "")
        {
            cout << UNKWOWN_COMMAND << endl;
            continue;
        }

        if (pieces.size() - 1 != current_command.parameter_number)
        {
            cout << WRONG_NUMBER_OF_PARAM << endl;
            continue;
        }

        if (current_command.is_exit)
        {
            cout << GREETING_AT_END << endl;
            return EXIT_SUCCESS;
        }
        ...
        if (not_numbers)
        {
            cout << ERROR_NOT_A_NUMBER << endl;
            continue;
        }
        cout << calculate(current_command.action, parameters.at(0), parameters.at(1)) << endl;


    }
}
double calculate(Funk f, double left, double right)
{
    double value = f(left, right);
