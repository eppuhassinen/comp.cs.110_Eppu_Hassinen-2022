...
bool write_data(const std::string& file_name,
    ...
    for (auto& n : alphabetical_order)
    {
        file_object << n.second->student_number << ";" << n.second->user_id
                  << ";" << n.second->name << ";"<< n.second->phone_number << ";"
                  << n.second->email << ";" << n.second->skype << std::endl;
    }
...
int main() {
    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            student_numbers.at(parts.at(1))->phone_number = new_phone_n;
